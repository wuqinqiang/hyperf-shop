	...
	<div class="add-commodity">
		...
		>
			...
			<FormItem label="商品排序" prop="sort">
				<Input v-model="formData.sort" class="form-item-component" />
			</FormItem>
...
export default {
	...
	watch: {
		categoryList(val) {
			if (val) {
				this.setCategory(this.formData.category_id)
			}
		}
	},
	methods: {
		...
		fareTypeChange(type) {
			if (type === 1) {
				this.formData.template_id = undefined
			} else if (type === 2) {
				this.formData.fare = undefined
			}
		},
		...
		handleSubmit() {
			this.$refs['eidtCommodityForm'].validate(valid => {
				if (valid) {
					if (
						!this.formData.image_address ||
				...
				if (item.id === categoryId) {
					...
					item.children.forEach(subItem => {
						if (subItem.id === categoryId) {
							this.parentCategoryId = item.id
							this.childCategoryId = subItem.id
						}
					})
				}
			})
		...
		getCommodityDetail() {
			http.get(`/product/${this.id}`).then(res => {
				...
				this.setCategory(res.data.category_id)
				this.formData.sort = res.data.sort
				this.formData.descriptions = res.data.descriptions
				this.formData.fare_type = res.data.fare_type
				this.formData.fare = parseFloat(res.data.fare)
				this.formData.template_id = res.data.template_id
				this.formData.on_sale = res.data.on_sale
				this.formData.time_on = res.data.time_on
				this.formData.time_off = res.data.time_off
				this.formData.skus = res.data.skus
				this.formData.front_image = res.data.front_image
				this.formData.description = res.data.description
				this.formData.only_buy = res.data.only_buy
				this.formData.only_count = res.data.only_count
				this.formData.image_address = res.data.images.map(item => {
	...
	created() {
		...
		this.id && this.getCommodityDetail()
	}
}
</script>
